...
import { resetPassword } from 'src/apiprovider/authentication/authentication'
...
const ResetPasswordV1 = () => {
  ...
  const updatePassword = () => {
    const token = encodeURIComponent(router.query?.token as string)
    if (token) {
      resetPassword(values.newPassword, values.confirmNewPassword, token)
        .then(res => {
          toast.success('Password changed successfully!');
          router.push('/login');
        })
        .catch(e => {
          setError(e.response.data?.password || e.response.data?.re_password || e.response.data?.non_field_errors || e.response.data?.messages || 'Something went wrong!')
          console.log(e)
        })
    } else {
      router.push('/login');
...
ResetPasswordV1.guestGuard = false
