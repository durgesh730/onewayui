...
import { Create_job, Fetch_company } from 'src/apiprovider/Job/Job';
import { getUserCompany } from 'src/context/selectCompany'
import Router from "next/router"

const Setup = ({ setactive }: any) => {
  ...
  const handleNext = () => {
    ...
    Create_job(postData)
      .then((response) => {
        ...
        localStorage.setItem('companyId', response.job.company);
        Router.push(`/jobs/${jobID}/edit/?activeStep=1`);
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  useEffect(() => {
    ...
    Fetch_company()
      .then((res) => {
        setCompanyId(getUserCompany(res).id);
      })
      .catch((error) => {
        console.error(error);
      });
      ...
      <Box padding={5} py={8}>
        <form>
          <Grid container spacing={5}>
            ...
            <Grid item xs={12}>
              <TextField fullWidth error={errordes} variant='outlined' label='Job Description*' multiline rows={2} onChange={handleChangeDescription} />
            </Grid>
            <Grid item xs={6}>
            ...
            <Grid item xs={12}>
              <FormControl fullWidth>
                ...
                >
                  ...
                  <MenuItem key="Australia/Hobart" value="Australia/Hobart">(GMT+10:00) Hobart</MenuItem>
