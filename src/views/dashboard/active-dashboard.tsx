...
import { archiveJob } from 'src/apiprovider/dashboard/jobs_view'
...
const JobCard = () => {
  ...
  useEffect(() => {
    async function fetchJobData() {
      try {
        ...
        const response = await activeJobs();
        setJobData(response);
  ...
  const handleRedirect = (jobId: string, stage: number) => {
    Router.push(`/jobs/${jobId}/workflow?stage=${stage}`)
  }
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6}>
                ...
                <Typography fontSize={'0.8rem'} textAlign={{ xs: 'center', sm: 'left' }}>
                  Total {job.total_candidates} Responded {job.responded_candidates} People ({job.response_percent.toFixed(2)}%)
                </Typography>
              </Grid>
              <Grid
                item
